...
'''
Metodo que me devuelve una password aleatorio
'''
def RandomPassword(size=6, is_strong=True):
    numbersandletters = string.digits + string.letters
    allcharacters = string.punctuation + numbersandletters
    if is_strong:
        return ''.join(random.SystemRandom().choice(allcharacters) for _ in range(size))
    else:
        return ''.join(random.SystemRandom().choice(numbersandletters) for _ in range(size))
