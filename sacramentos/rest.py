    ...
    if request.is_ajax():
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                libro.save()
                obj, created = Configuracion.objects.get_or_create(libro_bautismo=True)
...
def buscar_usuarios(request):
    if request.is_ajax():
        ...
        if cedula:
            try:
                if id_perfil:
                    perfiles = PerfilUsuario.objects.filter(dni=cedula).exclude(user__groups__name='Sacerdote').exclude(
                        pk=id_perfil).order_by('user__last_name', 'user__first_name')
                else:
                    perfiles = PerfilUsuario.objects.filter(dni=cedula).exclude(
                        user__groups__name='Sacerdote').order_by('user__last_name', 'user__first_name')
            ...
            try:
                ...
                if id_perfil:
                    perfiles = PerfilUsuario.objects.filter(user__last_name__icontains=apellidos,
                                                            user__first_name__icontains=nombres).exclude(
                        user__groups__name='Sacerdote').exclude(pk=id_perfil).order_by('user__last_name',
                                                                                       'user__first_name')
                else:
                    perfiles = PerfilUsuario.objects.filter(user__last_name__icontains=apellidos,
                                                            user__first_name__icontains=nombres).exclude(
    ...
    if request.is_ajax():
        ...
        if cedula:
            ...
            try:
                bandera = True
                perfiles = PerfilUsuario.objects.filter(user__last_name__icontains=apellidos,
                                                        user__first_name__icontains=nombres,
                                                        profesion='Sacerdote').order_by('user__last_name',
                                                                                        'user__first_name')
                if len(perfiles) > 0:
                    perfiles.distinct().order_by('user__last_name', 'user__first_name')
...
def data_tables(request):
    ...
    iDisplayLength = request.GET.get(
        'iDisplayLength')  # Obtiene el numero de registros que van a ser mostrados de un intervalo [5, 10, 25, Todos]
    ...
    for i in iSortingCols:
        ...
        columna = request.GET[
            columna_ordenable]  # devuelve verdadero si la columna es ordenable y falso si pasa lo contrario
        if columna:
            pass
    ...
    # q = 'Luis'
    if q:
        query = reduce(operator.__or__,
                       [Q(user__first_name__icontains=q) | Q(user__last_name__icontains=q) | Q(dni=q) for q in q])
    ...
    ctx = {"sEcho": sEcho, "iTotalRecords": iTotalRecords, "iTotalDisplayRecords": iTotalDisplayRecords,
           "aaData": lista}
    # print lista
    return HttpResponse(json.dumps(ctx), content_type='application/json')