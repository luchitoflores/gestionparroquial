...
class UsuarioAdministradorForm(ModelForm):
	...
	# 	data = self.cleaned_data.get('is_staff')
	# 	print 'el usuario logueado es: %s ' % self.user
	# 	return data
...
class BautismoForm(ModelForm):
	...
	def clean(self):
		...
		if persona.es_comunion(persona) or persona.es_confirmado(persona) or persona.es_novio(persona) or persona.es_novia(persona):
...
class AsignarParroquiaForm(ModelForm):
	persona = forms.ModelChoiceField(label = 'Sacerdote', queryset=PerfilUsuario.objects.sacerdote(), empty_label='-- Seleccione --') 
