...
class DatePickerInput(DateInput):
    # Always set input type to be "date"
    input_type = 'date'
    format_key = 'DATETIME_INPUT_FORMATS_EXTRA'
...
class UsuarioBaseForm(ModelForm):
    ...
    def clean_first_name(self):
        nombres = ''.join((c for c in unicodedata.normalize('NFD', unicode(self.cleaned_data.get('first_name'))) if
                           unicodedata.category(c) != 'Mn'))
        return nombres

    def clean_last_name(self):
        apellidos = ''.join((c for c in unicodedata.normalize('NFD', unicode(self.cleaned_data.get('last_name'))) if
                             unicodedata.category(c) != 'Mn'))
        return apellidos
...
class UsuarioForm(UsuarioBaseForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields[
            'groups'].help_text = 'Los grupos a los que este usuario pertenece. Un usuario obtendrá todos los permisos concedidos a cada uno sus grupos. Ud. puede seleccionar más de una opción.'
        if self.instance.id:
...
class PersonaBaseForm(ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'fecha_nacimiento': DatePickerInput(attrs={'required': '', 'data-date-format': 'dd/mm/yyyy', 'type': 'date'}),
...
class LibroBaseForm(ModelForm):
    class Meta:
        ...
        widgets = {
            'fecha_apertura': DatePickerInput(attrs={'required': '', 'data-date-format':
...
class LibroBautismoForm(LibroBaseForm):
    class Meta(LibroBaseForm.Meta):
        fields = LibroBaseForm.Meta.fields + ('tipo_libro',)

    def __init__(self, *args, **kwargs):
        self.request = kwargs.pop('request', None)
        super(LibroBautismoForm, self).__init__(*args, **kwargs)
        self.fields['tipo_libro'].initial = 'bautismo'


class LibroEucaristiaForm(LibroBaseForm):
    class Meta(LibroBaseForm.Meta):
        fields = LibroBaseForm.Meta.fields + ('tipo_libro',)

    def __init__(self, *args, **kwargs):
        self.request = kwargs.pop('request', None)
        super(LibroEucaristiaForm, self).__init__(*args, **kwargs)
        self.fields['tipo_libro'].initial = 'eucaristia'


class LibroConfirmacionForm(LibroBaseForm):
    class Meta(LibroBaseForm.Meta):
        fields = LibroBaseForm.Meta.fields + ('tipo_libro',)

    def __init__(self, *args, **kwargs):
        self.request = kwargs.pop('request', None)
        super(LibroConfirmacionForm, self).__init__(*args, **kwargs)
        self.fields['tipo_libro'].initial = 'confirmacion'


class LibroMatrimonioForm(LibroBaseForm):
    class Meta(LibroBaseForm.Meta):
        fields = LibroBaseForm.Meta.fields + ('tipo_libro',)

    def __init__(self, *args, **kwargs):
        self.request = kwargs.pop('request', None)
        super(LibroMatrimonioForm, self).__init__(*args, **kwargs)
        self.fields['tipo_libro'].initial = 'matrimonio'
...
class LibroForm(ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'fecha_cierre': DatePickerInput(attrs={'data-date-format': 'dd/mm/yyyy', 'type': 'date',
                                                   'label': 'Fecha Cierre *'}),
    ...
    def clean(self):
        ...
        if principal:
            ...
            if self.instance.id:
                if Libro.objects.filter(tipo_libro=tipo_libro, parroquia=parroquia, principal=True).exclude(
                        pk=self.instance.id):
...
class SacramentosForm(ModelForm):
    class Meta:
        ...
        widgets = {
            'fecha_sacramento': DatePickerInput(attrs={'required': '', 'data-date-format':
                'dd/mm/yyyy', 'type': 'date'}),
...
class BautismoForm(SacramentosForm):
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        if not self.instance.id:
            self.fields['bautizado'] = forms.ModelChoiceField(required=True, queryset=Bautismo.objects.none(),
                                                              ...
                                                              widget=forms.Select(attrs={'required': ''}),
                                                              error_messages={
                                                                  'invalid': 'El feligrés ya se encuentra bautizado, soy yo'})
        else:
    ...
    def clean(self):
        ...
        if not self.instance.id:
            ...
            elif Eucaristia.objects.filter(feligres=persona) or Confirmacion.objects.filter(
                    confirmado=persona) or Matrimonio.objects.filter(novio=persona) or Matrimonio.objects.filter(
                    novia=persona):
                self._errors['bautizado'] = self.error_class(
                    ["El feligres ya tiene un sacramento posterior al Bautismo"])
...
class EucaristiaForm(SacramentosForm):
    ...
    def clean(self):
        ...
        if not self.instance.id:
            ...
            elif Confirmacion.objects.filter(confirmado=persona) or Matrimonio.objects.filter(
                    novio=persona) or Matrimonio.objects.filter(novia=persona):
                self._errors['feligres'] = self.error_class(
                    ["El feligres ya tiene un sacramento posterior a la Primera Comunión"])
    ...
    def clean(self):
        ...
        if not self.instance.id:
            if Confirmacion.objects.filter(confirmado=persona).exists():
                ...
                self._errors['confirmado'] = self.error_class(
                    ["El feligres ya tiene un sacramento posterior a la Confirmación"])
...
class IntencionForm(ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'fecha': DatePickerInput(attrs={'required': '', 'data-date-format': 'dd/mm/yyyy'}),
...
class AsignarParroquiaForm(ModelForm):
    ...
    def clean(self):
        ...
        periodo_activo_otra_parroquia = PeriodoAsignacionParroquia.objects.filter(asignacion__parroquia=parroquia,
                                                                                  asignacion__persona__user__groups__name='Sacerdote',
                                                                                  estado=True).exclude(
            asignacion__persona=persona)
...
class PeriodoAsignacionParroquiaForm(ModelForm):
    class Meta:
        ...
        widgets = {
            'inicio': DatePickerInput(attrs={'required': '', 'data-date-format': 'dd/mm/yyyy', 'type': 'date'}),
            'fin': DatePickerInput(attrs={'data-date-format': 'dd/mm/yyyy', 'type': 'date'}),
        }
...
class EventoForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'fecha':  DatePickerInput(attrs={'data-date-format': 'yyyy-mm-dd', 'type': 'date'}),
